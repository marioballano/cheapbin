/*
 * ALSA audio backend for the SDL front-end on Linux (the GKD ARM64 target).
 * Same protocol as src/audio_linux.c — duplicated here so the SDL build
 * stays self-contained inside src/sdl/.
 */
...
{
    s_running = 1;
    if (pthread_create(&s_thread, NULL, audio_thread_func, NULL) != 0) {
...
void audio_pause(void)  { s_paused = 1; }
void audio_resume(void) { s_paused = 0; }
